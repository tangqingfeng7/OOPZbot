...
class ConsumeWebCommandTest(unittest.TestCase):
    ...
    def _handler(self, current_area) -> Any:
        ...
        handler._voice_channel_area = current_area
        handler._playback_lock = threading.RLock()
    ...
    def test_command_from_another_area_is_not_executed(self) -> None:
        ...
        command = AreaWebCommand(AreaId("area-B"), "next", {})
        self.assertFalse(handler._consume_web_command(encode_web_command(command)))
    ...
    def test_command_from_the_playing_area_is_executed(self) -> None:
        ...
        command = AreaWebCommand(AreaId("area-A"), "next", {})
        self.assertTrue(handler._consume_web_command(encode_web_command(command)))
        handler._execute_web_command.assert_called_once_with(
            command,
            queue=mock.sentinel.queue,
        )

    def test_global_volume_is_executed_anywhere(self) -> None:
        handler = self._handler("area-A")
        command = GlobalWebCommand("volume", {"value": 20})
    ...
    def test_bytes_payload_is_decoded(self) -> None:
        ...
        self.assertTrue(handler._consume_web_command(encode_web_command(command).encode()))
        handler._execute_web_command.assert_called_once_with(
            command,
            queue=mock.sentinel.queue,
        )
    ...
    def test_legacy_payload_is_dropped(self) -> None:
        ...
        self.assertFalse(handler._consume_web_command("area-A|next"))
        handler._execute_web_command.assert_not_called()
    ...
    def test_area_switch_cannot_redirect_an_inflight_command(self) -> None:
        handler = self._handler("area-A")
        ...
        def execute(command, queue=None):
            self.assertTrue(switch_attempted.wait(timeout=1))
            executed.append((handler._voice_channel_area, queue))
            release_command.set()
            return True
        ...
        def switch_area():
            ...
            with handler._playback_lock:
                handler._voice_channel_area = "area-B"
                handler._playback_generation += 1

        command_thread = threading.Thread(
            target=handler._consume_web_command,
            args=(encode_web_command(AreaWebCommand(AreaId("area-A"), "stop", {})),),
        )
        command_thread.start()
        switch_thread = threading.Thread(target=switch_area)
        switch_thread.start()
        self.assertTrue(release_command.wait(timeout=1))
        command_thread.join(timeout=1)
        switch_thread.join(timeout=1)

        self.assertEqual(executed, [("area-A", queue_a)])
        self.assertEqual(handler._voice_channel_area, "area-B")

    def test_stale_stream_task_cannot_play_in_new_area(self) -> None:
        from music.music import MusicHandler
        ...
        handler._stream_to_voice_channel(
    ...
    def test_stale_stream_task_cannot_override_new_song_in_same_area(self) -> None:
        from music.music import MusicHandler
