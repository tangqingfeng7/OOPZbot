...
class ChatInteractionService:
    ...
    async def handle_mention_fallback(self, text: str, channel: str, area: str, user: str = "") -> None:
        """处理 @bot 未匹配到已知指令时的 AI 兜底回复。支持上下文记忆。"""
        self._ensure_memory()

        history = None
        if self._memory and user:
            history = await self._memory.get_history(user, channel)

        reply = await self._chat.ai_reply(text, history=history)
        if reply:
            if self._memory and user:
                await self._memory.add_round(user, channel, text, reply)
        ...
        await self._sender.send_message("我没听懂，输入 @bot 帮助 查看指令", channel=channel, area=area)
    ...
    async def clear_memory(self, user: str, channel: str) -> bool:
        """清除指定用户在指定频道的对话记忆。"""
        self._ensure_memory()
        if self._memory:
            return await self._memory.clear(user, channel)
        return False

    async def clear_user_memory(self, user: str) -> int:
        """清除指定用户在所有频道的对话记忆。"""
        self._ensure_memory()
        if self._memory:
            return await self._memory.clear_user(user)
        return 0
