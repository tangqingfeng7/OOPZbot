...
class ChannelGroupInfo(BaseModel):
    ...
    # 本地修改（上游 bug，见根目录 issue-null-channels.md）：接口对空分组会返回
    # 显式的 `"channels": null`，而 `default_factory` 只在键缺失时生效，None 会直接
    # 撞上 list 校验抛 ValidationError。该异常发生在启动期的 populate_names，
    # 会导致 Bot 完全无法启动。
    @model_validator(mode="before")
    @classmethod
    def _tolerate_null_channels(cls, data: Any) -> Any:
        if isinstance(data, dict) and data.get("channels", False) is None:
            data = {**data, "channels": []}
        return data
